...
directory_out = fullfile(pwd,'results','simulated spectral blueing 2 lobe');
...
for i=1:wavelengthBleuing
    ...
    % calculate measured intensity in 2 PSF lobes
    lobe1(i) = sum(T_sys.*emission*F1,'all');
    lobe2(i) = sum(T_sys.*emission*F2,'all');
    ...
    subplot(2,2,3)
    plot(1:i,lobe1,'Color',col1,'LineWidth',1.5); hold on
    ...
    subplot(2,2,4)
    plot(1:i,lobe1./(lobe1+lobe2),'Color',col1,'LineWidth',1.5); hold on
    plot(1:i,lobe2./(lobe1+lobe2),'Color',col2,'LineWidth',1.5);
    xlim([0 wavelengthBleuing]); box off
    ylim([0 1])
...
lw = 1.5;
fontsize = 10;
legendNames{1} = strcat('\lambda_{ex} = ',sprintf(' %d ',wavelength1),' nm');
legendNames{2} = strcat('\lambda_{ex} = ',sprintf(' %d ',wavelength2),' nm');
...
xlim([0 inf]); ylim([0 inf]); box off
xlabel('Spectral shift, \Delta\lambda_{blueing} (nm)')
ylabel('Lobe intensity')
set(gca,'FontSize',fontsize)

subplot(1,2,2)
plot(1:wavelengthBleuing,lobe1./(lobe1+lobe2),'Color',col1,'LineWidth',lw); hold on
plot(1:wavelengthBleuing,lobe2./(lobe1+lobe2),'Color',col2,'LineWidth',lw);
legend(legendNames,'Location','northoutside','EdgeColor','none')
xlabel('Spectral shift, \Delta\lambda_{blueing} (nm)'); box off
ylabel('Lobe intensity/Total intensity')
set(gca,'FontSize',fontsize)
