...
directory_out = fullfile(pwd,'simulated spectral blueing 2 lobe');
...
for i=1:wavelengthBleuing
    ...
    lobe488(i) = sum(T_sys.*emission*F488,'all');
    lobe638(i) = sum(T_sys.*emission*F638,'all');
    ...
    subplot(2,2,3)
    plot(1:i,lobe488,'Color',col_488,'LineWidth',1.5); hold on
    ...
    subplot(2,2,4)
    plot(1:i,lobe488./(lobe488+lobe638),'Color',col_488,'LineWidth',1.5); hold on
    plot(1:i,lobe638./(lobe488+lobe638),'Color',col_638,'LineWidth',1.5);
    xlim([0 wavelengthBleuing]); box off
    ylim([0 1])
...
lw = 1.5;
fontsize = 10;
legendNames = {'\lambda_{ex} = 488 nm','\lambda_{ex} = 638 nm'};
...
subplot(1,2,2)
plot(1:wavelengthBleuing,lobe488./(lobe488+lobe638),'Color',col_488,'LineWidth',lw); hold on
plot(1:wavelengthBleuing,lobe638./(lobe488+lobe638),'Color',col_638,'LineWidth',lw);
legend(legendNames,'Location','northoutside','EdgeColor','none')
xlabel('Spectral shift, \Delta\lambda_{blueing} (nm)'); box off
ylabel('Lobe intensity/Total intensity')
set(gca,'FontSize',fontsize)
